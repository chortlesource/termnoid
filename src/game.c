...
void term_check_lines(struct game *game) {
  ...
  // Cycle through the buffer to find complete lines
  for(int y = 1; y < buffer_h; y++) {
    ...
    // If we have a complete line remove it
    if(llen == buffer_w - 2) {
      term_remove_line(game, y);
      ++lcount;
    }
...
void  term_move_down(struct game *game, float delta) {
  ...
  if(game->elapsed > game->speed) {
    if(term_is_valid_move(game, game->pos_x, game->pos_y + 1)) {
      game->pos_y += 1;
    } else {
      term_is_stuck(game);
      term_respawn(game);
...
void term_score(struct game *game, int lines) {
  ...
  int ten_percent = (game->level + 1 * 1000) / 10;

  if(game->score > (game->level * 500 + ten_percent)) {
    game->level += 1;
    game->speed -= 8;
  }
}
